...
pub struct Hal1210ClientCodec;

impl Decoder for Hal1210ClientCodec {
  type Item = MessageToClient;
  type Error = Error;
  ...
  fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
    ...
    tracing::trace!(target: "daemoncomm::client::decoder", "decoding {} bytes", len);
...
impl Encoder<MessageToServer> for Hal1210ClientCodec {
  type Error = Error;
  ...
  fn encode(&mut self, item: MessageToServer, dst: &mut BytesMut) -> Result<(), Self::Error> {
    ...
    tracing::trace!(target: "daemoncomm::client::encoder", "encoded {} bytes", len);
