...
fn main() -> miette::Result<()> {
  shader_build().expect("failed to build shaders");
  freenect_build()?;

  Ok(())
}

fn shader_build() -> Result<(), Box<dyn std::error::Error>> {
...
fn freenect_build() -> miette::Result<()> {
  ...
  let mut b = autocxx_build::Builder::new("src/kinect/freenect/mod.rs", [&include_path, &headers_path]).build()?;
  b.flag_if_supported("-std=c++14")
    .flag_if_supported("-Wno-mismatched-new-delete")
    .flag_if_supported("-Wno-unused-parameter")
    .compile("libfreenect");
  println!("cargo:rerun-if-changed=src/kinect/freenect/mod.rs");
